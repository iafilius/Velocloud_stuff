...
def load_config(config_file):
    ...
    config['start'] = os.getenv('START', config.get('start', "2025-02-17T01:00:00.594788627+01:00"))
    config['stop'] = os.getenv('STOP', config.get('stop', "2025-02-22T15:04:00.000000000+01:00"))
...
def fetch_data_worker(queue, session_params, url, EdgeID, enterpriseId, start, limit_flow):
    """
...
def main():
    ...
    fetcher = Process(target=fetch_data_worker, args=(queue, session_params, url, config['EdgeID'], config['enterpriseId'], config['start'], config['limit_flow']))
    fetcher.start()
    # Write data chunks to output file as they arrive
    output_filename = f"output-EdgeFlowVisibilityMetrics_{config['start'].replace(':', '-').replace(' ', '_')}_to_{config['stop'].replace(':', '-').replace(' ', '_')}.json"
